export default function BaseButton({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      className={`${active ? 'border-stone-300 bg-active-gray text-black' : disabled ? 'bg-gray-200/70 text-gray-400' : 'border-stone-200 bg-transparent text-gray-500'} group flex items-center justify-center rounded border p-2 transition-all duration-300 h-sm:p-1 ${disabled ? 'cursor-not-allowed hover:bg-gray-200' : 'cursor-pointer hover:bg-active-gray'} ${className}`}
    >
      <span
